 ...
 *
 * Uses ShedLock to ensure only one instance executes each job at a time,
 * preventing race conditions in multi-instance deployments.
 *
 * Schedule intervals:
 * - Auction start: Every 60 seconds (1 minute)
 * - Auction end: Every 60 seconds (1 minute)
 */
@Slf4j
@Component
...
public class AuctionScheduler {
     ...
     *
     * Runs every minute to check for auctions ready to begin.
     * Lock ensures only one instance processes this batch.
     *
     * Lock configuration:
     * - lockAtMostFor: 50 seconds (prevents deadlock if instance crashes)
     ...
     *
     * Runs every minute to check for auctions that should be closed.
     * Lock ensures only one instance processes this batch.
     *
     * Lock configuration:
     * - lockAtMostFor: 50 seconds (prevents deadlock if instance crashes)
