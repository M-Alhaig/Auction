...
/**
 * Service interface for managing auction items.
...
public interface ItemService {
     ...
     * Sets initial status to PENDING and currentPrice = startingPrice.
     *
     ...
     * @throws IllegalStateException if item status is not PENDING
     */
     ...
     *
     * @param status the item status to filter by (PENDING, ACTIVE, SOLD)
    ...
    /**
     * Get all items created by a specific seller with pagination.
     * Used for seller dashboard to view their listings.
     *
    ...
    /**
     * Get items filtered by both seller and status with pagination.
     * Allows sellers to filter their own items (e.g., "show my active auctions").
     *
     * @param sellerId the UUID of the seller
    ...
    /**
     * Get active auctions ordered by end time (ending soonest first).
     * Used for homepage "Ending Soon" section.
     *
     * @param pageable pagination and sorting parameters
