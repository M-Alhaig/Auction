package com.auction.item_service.repositories;
...
public interface ItemRepository extends JpaRepository<Item, Long> {
    ...
    /**
     * Find all auction items created by a specific seller with pagination support.
     * Used for seller dashboard to view their own listings.
     *
    ...
    /**
     * Find all items with a specific status (PENDING, ACTIVE, SOLD) with pagination.
    ...
    /**
     * Find items by status, ordered by end time in ascending order (ending soonest first).
     * Useful for showing users which auctions are about to close.
     *
     * @param status the item status to filter by
     * @param pageable pagination and sorting parameters
    ...
    /**
     * Find all items with a specific status that end before the given time.
     * Critical for the auction scheduler to identify auctions that need to be closed.
     * Returns List (not Page) since scheduler must process ALL matching records.
     *
    ...
    /**
     * Find all items with a specific status that should start at or before the given time.
     * Used by scheduler to transition PENDING auctions to ACTIVE status.
     * Returns List (not Page) since scheduler must process ALL matching records.
     *
    ...
    /**
     * Find items by both seller and status with pagination support.
     * Allows sellers to filter their own items (e.g., "show me my active auctions").
     *
     * @param sellerId the UUID of the seller
     * @param status the item status to filter by
     * @param pageable pagination and sorting parameters
    ...
    /**
     * Check if an item exists and is currently ACTIVE.
