 ...
 * <p>
 * Implementation Strategy: - Current: RabbitMQEventPublisher (uses RabbitTemplate) - Future:
 * SQSEventPublisher (uses AWS SDK SQS/SNS clients)
 * <p>
 * Migration Path: To migrate from RabbitMQ to AWS SQS/SNS: 1. Implement SQSEventPublisher
...
public interface EventPublisher {
   ...
   * <p>
   * RabbitMQ Implementation: - Routes to exchange based on event class name - Uses topic exchange
   * with routing key pattern: {service}.{event-type}
   * <p>
   * SQS/SNS Implementation: - Publishes to SNS topic based on event class name - Subscribers (SQS
