...
/**                                                                                                                                                           │ │
  ...
  * Uses Spring AMQP RabbitTemplate to publish events to a topic exchange.
  *
  * Exchange Strategy:
...
public class RabbitMQEventPublisher implements EventPublisher{
    ...
    public <T> void publish(T event, String routingKey) {
        try {
            ...
            log.debug("Publishing event - exchange: {}, routingKey: {}, event: {}",
                    EXCHANGE_NAME, routingKey, event.getClass().getSimpleName());
            ...
            log.error("Failed to serialize event - type: {}, error: {}",
                    event.getClass().getSimpleName(), e.getMessage(), e);
            throw new EventPublishException("Event serialization failed: " + event.getClass().getSimpleName(), e);
        } catch (Exception e) {
            log.error("Failed to publish event - type: {}, routingKey: {}, error: {}",
                    event.getClass().getSimpleName(), routingKey, e.getMessage(), e);
            throw new EventPublishException("Event publishing failed: " + event.getClass().getSimpleName(), e);
        }
    }
