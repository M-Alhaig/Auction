...
public interface ItemLifecycleService {
  ...
  /**
   * Transition a single item from PENDING to ACTIVE status. Called when auction start time is
   * reached. Publishes AuctionStartedEvent to message queue.
   *
   * @param itemId the ID of the item to start
   * @throws com.auction.itemservice.exceptions.ItemNotFoundException if item doesn't exist
   * @throws IllegalStateException                                     if item is not in PENDING
   *                                                                   status
   */
  void startAuction(Long itemId);

  /**
   * Transition a single item from ACTIVE to Ended status. Called when auction end time is reached.
   * Publishes AuctionEndedEvent to message queue with winner information.
   *
   * @param itemId the ID of the item to end
   * @throws com.auction.itemservice.exceptions.ItemNotFoundException if item doesn't exist
   * @throws IllegalStateException                                     if item is not in ACTIVE
   *                                                                   status
   */
  ...
  /**
   * Find and start all PENDING auctions whose start time has been reached. Called by scheduled job
   * every minute. Processes all matching items in a single batch for efficiency.
   *
  ...
  /**
   * Update the current price of an item with distributed locking. Called when BidPlacedEvent is
   * received from bidding-service. Uses Redis distributed lock to prevent race conditions. Only
   * updates if newPrice > currentPrice.
   *
   * @param itemId   the ID of the item
   * @param newPrice the new current price from the bid
   * @throws com.auction.itemservice.exceptions.ItemNotFoundException  if item doesn't exist
   * @throws com.auction.itemservice.exceptions.ConcurrentBidException if lock cannot be acquired
   *                                                                    (retry suggested)
  ...
  /**
   * Check if an item exists and is currently in ACTIVE status. Used by bidding-service to validate
   * bids before processing. Efficient boolean check without loading full entity.
   *
  ...
  /**
   * Find all PENDING items whose start time has been reached. Used internally by
   * batchStartPendingAuctions().
   *
  ...
  /**
   * Find all ACTIVE items whose end time has been reached. Used internally by
   * batchEndExpiredAuctions().
   *
