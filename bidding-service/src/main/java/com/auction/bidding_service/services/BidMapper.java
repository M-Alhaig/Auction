 ...
 * - Determines if a bid is the current highest (requires context from service layer)
 *
 * Note: No DTO → Entity mapping needed. Bids are created directly in service layer
 * to ensure proper validation and Redis locking before persistence.
 */
@Component
public class BidMapper {
    ...
    /**
     * Convenience method for converting a bid that is definitely the current highest.
     * Used when we know for certain this bid is the winner (e.g., just placed successfully).
     */
    ...
    /**
     * Convenience method for converting a bid that is NOT the current highest.
     * Used for historical bids in bid history queries.
     */
